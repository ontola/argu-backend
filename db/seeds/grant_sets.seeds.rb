...
%w[Page Forum Blog OpenDataPortal Question Motion LinkedRecord ConArgument ProArgument
...
show_actions =
  %i[page_show forum_show blog_show open_data_portal_show question_show motion_show linked_record_show pro_argument_show
     con_argument_show comment_show vote_event_show vote_show blog_post_show decision_show].map { |a| @actions[a] }
...
administrate.permitted_actions <<
  %i[question_create motion_create pro_argument_create con_argument_create comment_create
     vote_create blog_post_create decision_create].map { |a| @actions[a] }
administrate.permitted_actions <<
  %i[page_update forum_update blog_update question_update motion_update
...
staff.permitted_actions <<
  %i[forum_create blog_create question_create motion_create pro_argument_create con_argument_create
...
staff.permitted_actions <<
  %i[question_trash motion_trash pro_argument_trash con_argument_trash
     blog_post_trash comment_trash].map { |a| @actions[a] }
staff.permitted_actions <<
  %i[page_destroy forum_destroy blog_destroy open_data_portal_destroy question_destroy motion_destroy
     pro_argument_destroy con_argument_destroy blog_post_destroy comment_destroy].map { |a| @actions[a] }
