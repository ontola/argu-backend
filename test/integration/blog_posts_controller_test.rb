...
class BlogPostsControllerTest < ActionDispatch::IntegrationTest
  ...
  define_tests do
    ...
    define_test(hash, :show, case_suffix: ' unpublished', options: {record: :scheduled}) do
    ...
    define_test(hash, :update, case_suffix: ' erroneous', options: {attributes: {title: 'BlogPost', content: 'C'}}) do
      {manager: {should: false, response: 200, asserts: [assert_has_title, assert_has_content]}}
    end
    options = {
      record: :scheduled,
    ...
    define_test(hash, :update, case_suffix: ' cancel schedule', options: options) do
