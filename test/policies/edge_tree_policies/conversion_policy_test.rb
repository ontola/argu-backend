...
class ConversionPolicyTest < PolicyTest
  subject { Conversion.new(edge: motion.edge, klass: Question) }
  let(:invalid_edge_subject) { Conversion.new(edge: argument.edge, klass: Motion) }
  let(:invalid_klass_subject) { Conversion.new(edge: motion.edge, klass: Argument) }
