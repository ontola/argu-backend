...
class FollowersCollectorTest < ActiveSupport::TestCase
  ...
  test 'should collect 0 for motion in unfollowed forum' do
    ...
    assert_equal 0, FollowersCollector.new(activity: motion.activities.second).call.count
  ...
  test 'should not collect notified followers for motion' do
    motion
    assert_equal 0, FollowersCollector.new(activity: motion.activities.second).call.count
    assert_equal 2, FollowersCollector.new(activity: motion.activities.second).count
  end

  test 'should collect reaction followers for motion' do
    motion
    Notification.destroy_all
    assert_equal 1, FollowersCollector.new(activity: motion.activities.second).call.count
    assert_equal 3, FollowersCollector.new(activity: motion.activities.second).count
  end

  test 'should not collect notified followers for motion with marked_as_important' do
    important_motion
    assert_equal 0, FollowersCollector.new(activity: important_motion.activities.second).call.count
  ...
  test 'should collect reaction and news followers for motion with marked_as_important' do
    important_motion
    Notification.destroy_all
    assert_equal 2, FollowersCollector.new(activity: important_motion.activities.second).call.count
    assert_equal 4, FollowersCollector.new(activity: important_motion.activities.second).count
  end
end
