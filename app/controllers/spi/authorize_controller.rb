  ...
  class AuthorizeController < SPI::SPIController
    ...
    def resource # rubocop:disable Metrics/AbcSize, Metrics/MethodLength
      ...
      when 'CurrentActor'
        profile = if (/[a-zA-Z]/i =~ params[:resource_id].to_s).present?
                    LinkedRails.iri_mapper.resource_from_iri(params[:resource_id], user_context)&.profile
                  else
    ...
    def resource_from_iri_param
      ActsAsTenant.with_tenant(TenantFinder.from_url(params[:resource_iri].to_s)) do
        LinkedRails.iri_mapper.resource_from_iri(path_to_url(resource_iri_param), user_context)
      end
    end
