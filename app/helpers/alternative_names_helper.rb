...
module AlternativeNamesHelper
  ...
  def motion_icon(forum= nil)
    ((forum || current_forum).motions_title_singular if alternative_motions?(forum)) || 'lightbulb-o'
  end

  # Singular translation for {Motion}
  def motion_type(forum= nil)
    ((forum || current_forum).motions_title_singular if alternative_motions?(forum)) || I18n.t('motions.type')
  end

  # Plural translation for {Motion}
  def motions_type(forum= nil)
    ((forum || current_forum).motions_title if alternative_motions?(forum)) || I18n.t('motions.plural')
  end
  ...
  def alternative_motions?(forum= nil)
    if forum == nil
      @alternative_motions ||= current_forum.uses_alternative_names &&
          current_forum.motions_title_singular.present? &&
          current_forum.motions_title.present?
    else
  ...
  def question_icon(forum= nil)
    ((forum || current_forum).questions_title_singular if alternative_questions?(forum)) || 'question'
  end

  # Singular translation for {Question}
  def question_type(forum= nil)
    ((forum || current_forum).questions_title_singular if alternative_questions?(forum)) || I18n.t('questions.type')
  end

  # Plural translation for {Question}
  def questions_type(forum= nil)
    ((forum || current_forum).questions_title if alternative_questions?(forum)) || I18n.t('questions.type')
  end
  ...
  def alternative_questions?(forum= nil)
    if forum == nil
      @alternative_questions ||= current_forum.uses_alternative_names &&
          current_forum.questions_title_singular.present? &&
          current_forum.questions_title.present?
    else
      forum.uses_alternative_names &&
  ...
  def argument_icon(forum= nil)
    ((forum || current_forum).arguments_title_singular if alternative_arguments?(forum)) || 'argument'
  end

  # Singular translation for {Argument}
  def argument_type(forum= nil)
    ((forum || current_forum).arguments_title_singular if alternative_arguments?(forum)) || I18n.t('arguments.type')
  end

  # Plural translation for {Argument}
  def arguments_type(forum= nil)
    ((forum || current_forum).arguments_title if alternative_arguments?(forum)) || I18n.t('arguments.type')
  end
  ...
  def alternative_arguments?(forum= nil)
    if forum == nil
      @alternative_arguments ||= current_forum.uses_alternative_names &&
          current_forum.arguments_title_singular.present? &&
          current_forum.arguments_title.present?
    else
      forum.uses_alternative_names &&
