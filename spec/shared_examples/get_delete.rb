    ...
    context "as #{format}" do
      unless opts[:skip].include?(:delete_guest) || opts[:skip].include?(:guest)
        it 'as guest' do
          ...
          get delete_path, headers: {accept: Mime::Type.lookup_by_extension(format).to_s}
      ...
      unless opts[:skip].include?(:delete_non_existing) || opts[:skip].include?(:non_existing)
        it 'non existing' do
          sign_in(authorized_user, doorkeeper_application)
          get non_existing_delete_path, headers: {accept: Mime::Type.lookup_by_extension(format).to_s}
        end
      end
    end
  end
