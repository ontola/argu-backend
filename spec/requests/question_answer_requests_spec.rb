...
RSpec.describe 'QuestionAnswers', type: :request do
  ...
  let(:non_existing_new_path) { new_question_answer_path(question_answer: {question_id: -1}) }
  ...
  let(:non_existing_create_params) { {question_answer: {question_id: -1, motion_id: forum_motion.id}} }
  let(:invalid_create_params) { {question_answer: {question_id: question.id, motion_id: -1}} }
