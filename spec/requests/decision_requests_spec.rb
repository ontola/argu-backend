...
RSpec.describe 'Decisions', type: :request do
  ...
  let(:non_existing_edit_path) { url_for([:edit, subject.parent_model, :decision, id: -99, only_path: true]) }
  let(:non_existing_show_path) { url_for([subject.parent_model, :decision, id: -99, only_path: true]) }
  let(:non_existing_create_path) do
    url_for([parent_class_sym, table_sym, "#{parent_class_sym}_id".to_sym => -99, only_path: true])
  end
  let(:non_existing_update_path) { url_for([subject.parent_model, :decision, id: -99, only_path: true]) }
